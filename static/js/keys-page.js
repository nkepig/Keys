window.keysApp = function keysApp() {
	...
	return {
		...
		get providerOptions() {
			...
			["OpenAI", "Anthropic", "Google", "OpenRouter", "xAI"].forEach((p) =>
				s.add(p),
			);
			return [...s].sort((a, b) => {
				const order = {
		...
		get uploadDupCount() {
			const lines = this.uploadForm.keys
				.split("\n")
				.map((l) => l.trim())
				.filter(Boolean);
			const seen = new Set();
		...
		get processedKeys() {
			...
			if (this.statusFilter !== "all") {
				...
				if (this.statusFilter === "other") {
					...
					result = result.filter(
						(k) => pinnedIds.has(k.id) || k.status_code === code,
					);
				}
			}
		...
		toggleActionMenu(menuName) {
			this.activeActionMenu =
				this.activeActionMenu === menuName ? null : menuName;
		},
		closeActionMenu() {
			this.activeActionMenu = null;
		},
		...
		async doUpload() {
			...
			const rawLines = this.uploadForm.keys
				.split("\n")
				.map((l) => l.trim())
				.filter(Boolean);
			const seen = new Set();
			...
			const timer = setInterval(() => {
				this.uploadProgress = Math.min(
					92,
					((Date.now() - startTime) / estimated) * 100,
				);
			}, 120);
			try {
		...
		async doUrlBatch() {
			...
			const lines = this.urlBatchForm.urlsText
				.split("\n")
				.map((l) => l.trim())
				.filter(Boolean);
			if (!lines.length) {
			...
			const startTime = Date.now();
			const timer = setInterval(() => {
				this.urlBatchProgress = Math.min(
					92,
					((Date.now() - startTime) / estimated) * 100,
				);
			}, 120);
